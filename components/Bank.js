...
function Copyable({ title, content, classNames }) {
    ...
    return (
        <div className={`grid grid-cols-2 border-t border-tertiary p-4 ${classNames ? classNames : ''}`}>
            <div>{title}</div>
            <div className="font-sans font-medium flex justify-between cursor-pointer" onClick={() => copyToClipboard(content)}>
                <div className="font-sans font-medium">{content}</div>
                {   isCopied ? 
